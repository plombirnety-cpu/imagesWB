...
_MAGAZINE_PRINT_PROMPT_SUFFIX = (
    ...
    "light outer border may join them to the character. NEVER place one shared "
    "backing, blob or plate OF ANY COLOUR behind or connecting the character, "
    "typography and effects: bare chroma must remain visible between independent "
    "elements. NEVER draw a white/cream/light contour "
...
def _generation_model_for_design(design: dict) -> str | None:
    """Премиальная image-модель для детальных ручных production-стилей."""
    if (
