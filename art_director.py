...
_MAGAZINE_COVER_QUALITY_HINT = (
    ...
    "Торс заканчивается ВЫШЕ BOTTOM EDGE, эффект формирует фигурный die-cut низ, а под "
    ...
    "персонажа, текст и эффекты общей белой/кремовой/бежевой sticker-подложкой: светлый "
    "цвет разрешён только локально в волосах, одежде, буквах и тонких keyline-контурах, "
    "а промежутки между отдельными элементами обязаны оставаться чистым хромакеем."
)
...
def _ask_claude(theme: str, n: int, fmt: str, recent_styles: list = None,
    ...
    # Стилевые code-level рычаги: gym получает вариативность, журнальная обложка —
    # стабильную утверждённую сетку и качество крупного портрета.
    user = user + _gym_variety_hint(style_pref) + _magazine_cover_quality_hint(style_pref)
