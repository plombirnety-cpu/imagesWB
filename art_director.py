...
_DIECUT_BODY = (
    "ФОРМАТ: фигурный мерч-принт с ОТКРЫТЫМ контуром, НЕ стикер и НЕ наклейка. Персонаж "
    "крупно; стилизованное пламя/энергия/аура (цвет и характер эффекта — под тему "
    "персонажа: огонь, молнии, чакра, тёмная энергия и т.п.) асимметрично поддерживает "
    "плечи и низ фигуры, но НЕ окружает её со всех сторон, НЕ замыкается в кольцо и НЕ "
    "образует общий cutline/ореол/светлую кайму. Края персонажа и локальных эффектов "
    "напрямую соприкасаются с хромакеем; между отдельными языками эффекта виден фон. "
    "Не создавай позади персонажа, текста и эффектов одну общую соединяющую их "
    "подложку/blob/plate ЛЮБОГО цвета: независимые элементы разделяет голый хромакей. "
    "Ниже персонажа и эффектов должно остаться пустое "
...
_MAGAZINE_COVER_STYLE_ID = "34_anime_magazine_cover"
_MAGAZINE_COVER_QUALITY_HINT = (
    " ДЛЯ ЭТОГО дизайна строго соблюдай утверждённую сетку streetwear-ПРИНТА, "
    "вдохновлённого журнальной графикой, NOT A RECTANGULAR magazine cover/card: "
    "ВЕРТИКАЛЬ 2:3; один канонически узнаваемый персонаж крупным портретом по грудь "
    "занимает 65-78% высоты; лицо резкое и красивое, руки вне кадра; огромный "
    "катакана-заголовок сверху, 2-3 короткие строки слева, вертикальные кандзи справа, "
    "печати и короткая цитата снизу — ни один текст не закрывает глаза, нос или рот. "
    "Сохрани точные канонические цвета волос, глаз и костюма из character_ref. "
    "Обязателен SIGNATURE-EFFECT CRADLE: канонический эффект героя (огонь, молнии, "
    "тени, лёд, ветер, лепестки, энергия) обнимает плечи и НИЖНИЙ контур бюста "
    "асимметричными языками/дугами/частицами, как огонь у утверждённого Тандзиро. "
    "Этот эффект остаётся ОТКРЫТЫМ: он НЕ продолжается в 360-градусное кольцо вокруг "
    "персонажа, НЕ образует cutline, halo, badge edge или замкнутую светлую кайму. "
    "Торс заканчивается ВЫШЕ BOTTOM EDGE, эффект формирует асимметричный фигурный низ, а под "
    "ним и в обоих нижних углах остаётся чистый CHROMA MOAT. Все элементы, включая "
    "текст, держи на 6-8% от КАЖДОГО края: ничто не обрезано краем, никакой full-bleed "
    "страницы. Энергия героя выражается взглядом, волосами и этим эффектом, а НЕ "
    "широкой боевой сценой, мелкой фигурой или сложными кистями. Вся свободная площадь "
    "и каждый край холста — идеально ровный выбранный green/blue chroma без градиента, "
    "текстуры, комнаты, пейзажа, бумажной карточки или рамки. НИКОГДА не объединяй "
    "персонажа, текст и эффекты общей sticker-подложкой/blob/plate ЛЮБОГО цвета — между "
    "независимыми элементами обязан быть виден голый хромакей. НИКОГДА "
    "не проводи белый, кремовый или иной светлый контур по внешнему силуэту персонажа "
    "либо эффектов: светлый цвет разрешён только ВНУТРИ волос, одежды и букв; внешняя "
    "граница напрямую встречается с хромакеем, а промежутки между отдельными элементами "
    "обязаны оставаться чистым хромакеем."
)


def _magazine_cover_quality_hint(style_pref: str) -> str:
    """Жёсткая композиционная сетка утверждённых пользователем пробников стиля 34."""
    return _MAGAZINE_COVER_QUALITY_HINT if style_pref == _MAGAZINE_COVER_STYLE_ID else ""


_AUTOMOTIVE_EDITORIAL_STYLE_ID = "37_auto_racing_editorial"
_AUTOMOTIVE_EDITORIAL_QUALITY_HINT = (
    " ДЛЯ ЭТОГО автономного автомобильного дизайна САМ выбери РОВНО ОДНУ конкретную "
    "реальную машину или гоночный сюжет внутри заданной категории. Это НЕ абстрактный "
    "generic race car: зафиксируй узнаваемую геометрию, эпоху и цветовую идентичность. "
    "has_human_figure=false, character_en и title_en оставь пустыми. Придумай не "
    "рекламный шум, а СМЫСЛОВУЮ текстовую иерархию: одно безошибочное крупное имя "
    "модели/команды/серии, одна короткая связанная с ней фраза 2-5 слов и только 2-4 "
    "малых факта (номер, год, страна, город/трасса, класс) — включай точный факт лишь "
    "когда уверен, иначе используй честную ненумерическую формулировку. type_spec обязан "
    "перечислить точные строки текста и их места. Машина крупная, в 3/4 или низком "
    "боковом ракурсе, может перекрывать часть wordmark; добавь максимум одну линию "
    "трассы, флаг и сдержанные стрелки/timing marks. Все элементы — отдельные острова "
    "на чистом chroma с открытыми промежутками. НИКАКОЙ прямоугольной фотографии, дороги, "
    "гаража, белого/чёрного листа, общей подложки, стены спонсоров или белой sticker-"
    "обводки вокруг машины. NEUTRAL DUAL-CONTRAST TYPE обязателен для ВСЕХ крупных и "
    "малых надписей: neutral silver/mid-grey или warm greige fill, тонкий charcoal inner "
    "stroke И тонкий warm-ivory outer keyline одновременно. Такая типографика должна "
    "читаться и на чёрной, и на белой футболке. Цвет кузова/бренда разрешён в тексте "
    "только как крошечный акцент, но не как заливка основной надписи."
)


def _automotive_editorial_quality_hint(style_pref: str) -> str:
    """Точный автомобильный бриф для автономного style 37."""
    if style_pref == _AUTOMOTIVE_EDITORIAL_STYLE_ID:
        return _AUTOMOTIVE_EDITORIAL_QUALITY_HINT
    return ""
...
_CITY_NEOCREST_QUALITY_HINT = (
    " ДЛЯ ЭТОГО дизайна трактуй запрос как ОДИН реальный город, не персонажа и не "
    "франшизу. Верни quote РОВНО равным русскому названию города из запроса в "
    "ВЕРХНЕМ РЕГИСТРЕ; это единственный обязательный текст и он встречается один "
    "раз. slogan, kana, name_jp, character_en, title_en, signature_props и style_mix "
    "оставь пустыми; has_human_figure=false. Это RICH CITY NEO-CREST V2, плотный "
    "ювелирно-яркий принт, а не разреженный тёмный badge. Собери 7-10 крупных "
    "модулей: один точный главный ориентир, РОВНО два самостоятельных узнаваемых "
    "вспомогательных, один правдивый географический мотив, один местный транспортный "
    "или инженерный мотив и 2-4 широкие декоративные связки. Не более трёх зданий; "
    "не смешивай мосты, храмы, башни или дворцы в выдуманное здание. Заполни цветным "
    "непрозрачным artwork 86-92% внутреннего силуэта; никаких chroma- или почти "
    "чёрных дыр крупнее 4-5%. Построй четыре перекрывающихся плана: яркое солнце, "
    "кольцо, небо или орнамент сзади; второй городской слой; главный ансамбль; "
    "широкий foreground перед плашкой. Artwork занимает 74-78% высоты, название — "
    "нижние 22-26%. Используй 8-9 печатных цветов и минимум четыре крупные "
    "насыщенные цветовые массы; средние и светлые цвета занимают не менее 65%, "
    "near-black только контуром и не более 15%, все тёмные массы вместе не более "
    "25%. Чёрная внутренняя подложка и серо-коричневая доминанта запрещены. Каждый "
    "крупный элемент имеет базовый цвет, жёсткий блик и цветную жёсткую тень под "
    "единым светом сверху слева. Все буквы только настоящая русская кириллица: ни "
    "одной латинской буквы, перевода, даты, цифры, микротекста или псевдонадписи. "
    "Запрещены гигантский одинокий мост или башня, generic clock tower/skyscraper, "
    "пустой sunburst, повторяемый симметричный trophy badge, случайный skyline, "
    "прямоугольный постер, фото, герб, флаг, официальный знак, белая общая подложка, "
    "sticker-cutline, фотореализм, CGI-глянец, мягкие градиенты, филигрань и шум."
)


def _city_neocrest_quality_hint(style_pref: str) -> str:
    """Архитектурный и кириллический контракт ручного городского style 41."""
    if style_pref == _CITY_NEOCREST_STYLE_ID:
        return _CITY_NEOCREST_QUALITY_HINT
    return ""
...
def _ask_claude(theme: str, n: int, fmt: str, recent_styles: list = None,
    ...
    # Стилевые code-level рычаги: gym получает вариативность, журнальный и
    # автомобильный стили — стабильную утверждённую сетку.
    user = (
        user
...
def _style_bank_prompt_block(design: dict) -> str:
    ...
    if style_id == _CITY_NEOCREST_STYLE_ID:
        # Для насыщенного городского v2 одних essence/text_treatment недостаточно:
        # image-модель иначе снова сводит палитру к тёмному двухцветному badge.
