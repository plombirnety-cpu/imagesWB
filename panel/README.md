...
командной строки. Тонкая FastAPI-обёртка: движок (art_director, franchise_scout,
batch_print) не меняется, панель только раскладывает count по стилям/персонажам,
зовёт движок в фоне и отдаёт превью + ZIP.
...
green_only=True)` — ОДИН PNG на хромакее (зелёном/синем), без вырезки и
апскейла (апскейл владелец делает отдельной программой). Ошибка одного дизайна
не роняет весь job — попадает в `items[].error`, остальные продолжают
генерироваться.
...
`tests/test_orchestrator.py` мокает `art_director.make_ideas` и
`batch_print.render_design` (и `franchise_scout.build_dossier` для ветки 2) —
проверяет все 3 ветки логики оркестрации, санитайзинг тегов и сборку списка
результатов. Реальный сквозной прогон с живым Gemini — отдельная ручная проверка
(каждый `render_design` — платный вызов).
...
- Апскейл до печатного размера **сознательно выключен** (`UPSCALE=off`,
  `green_only=True`) — панель отдаёт превью-разрешение для отбора, финальный
  апскейл владелец делает отдельно (см. `PLAN.md`).
